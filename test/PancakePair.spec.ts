...
describe('PancakePair', () => {
  ...
  it('feeTo:on', async () => {
    ...
    await pair.burn(wallet.address, overrides)
    // TODO:
    // expect(await pair.totalSupply()).to.eq(MINIMUM_LIQUIDITY.add('479521068744441'))
    expect(await pair.balanceOf(other.address)).to.eq('479521068742441')

    // using 1000 here instead of the symbolic MINIMUM_LIQUIDITY because the amounts only happen to be equal...
    // ...because the initial liquidity amounts were equal
    // TODO:
    // expect(await token0.balanceOf(pair.address)).to.eq(bigNumberify(1000).add('479043232700094'))
